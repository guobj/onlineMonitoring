        ...
        $(function () {
            $.ajax({
                ...
                success:function (data) {
                    console.log(data.data.devType)
                    for(var type in data.data.devType){
                        console.log(data.data.devType.data_name)
                        $("#dev_type").append("<option value="+data.data.devType[type].data_value+">"+data.data.devType[type].data_name+"</option>");
                    }
                    for(var target in data.data.devType1){
                        $("#dev_target").append("<option value="+data.data.devType1[target].data_value+">"+data.data.devType1[target].data_name+"</option>");
                    }
                }
            });
        });
...
<div id="content_r">
    ...
    <div class="menu">
        <form action="queryDeviceInfoList" method="post">
            ...
            <span>监测对象：
                <select class="select1" name="dev_target" id="dev_target">
